...
- [ ] Build Stripe checkout for premium payments - *checked 22 Sep, NOT ticked. The checkout is built: `client/src/pages/checkout.tsx` + `PaymentForm` call `POST /api/payments/create-subscription` (`server/http/paymentRoutes.ts`) and confirm with Stripe Elements. But the server charges whatever `annualPremiumCents` the client posts, provided it sits between 10000 and 500000. It does not recompute the premium or check it against the Root quote, so an edited request can buy cover at £100 a year. `6853a80` made checkout display the Root premium; it did not make the server enforce it. Pricing is a guarded area for the nightly job, so this needs Jamal.*
...
- [x] Add premium amount display on policy page - *this was stale: both surfaces already show the real premium. `client/src/pages/policy.tsx` renders `formatCurrency(premiumAmount)` against an "Annual Premium" row (`premiumCents` from `policy.currentPremiumCents` or `userDoc.activePolicy.premiumCents`, no hardcoded figure), landed with the Wave G policy-terms fix (`9b6fbae`) and confirmed live through the refund-cap fix (`7d11cc0`). `mobile/app/policy.tsx` shows the same field as "Premium" via `formatPounds(policy.premiumCents)`. Ticked 2026-09-15, no code changed - see `nightly/2026-09-15`.*
...
- [x] Split `server/routes.ts` into domain-specific route modules - *done: `bee3ef3` (7 Sep), ticked `nightly/2026-09-21`. `server/routes.ts` went 1520 to 43 lines and now only mounts auth middleware, the health check and six `register*Routes` calls; the handlers live in `server/http/` as `authRoutes`, `driverRoutes`, `communityRoutes`, `aiRoutes`, `paymentRoutes` and `webhookRoutes`, none over 364 lines. The refactor never ticked its own ticket.*
- [x] Add OpenAPI documentation for Express API - *done: `nightly/2026-09-22`. `docs/api/openapi.json` (OpenAPI 3.0.3) covers all 38 `/api` operations `server/routes.ts` registers, with bearer auth, path/query parameters, the named body fields each handler reads, and the status codes it can answer. Response bodies stay open objects until the routes have typed contracts; inventing shapes would make the doc a second source of truth that nothing checks. Held by `server/__tests__/openapi-coverage.test.ts`, which walks the live Express router, so a route added or removed in any module, or a route whose `requireAuth` changes, fails the suite until the spec follows.*
